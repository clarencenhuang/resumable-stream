...
import { createPublisherAdapter, createSubscriberAdapter } from "./postgres-adapters";
 ...
 */
export interface PostgresCreateResumableStreamContextOptions
...
export function createResumableStreamContext(
  options: PostgresCreateResumableStreamContextOptions
): ResumableStreamContext {
  const { sql, tableName, ...restOptions } = options;

  // Create adapters using the provided sql instance and table name
  const publisher = createPublisherAdapter(sql, tableName);
  const subscriber = createSubscriberAdapter(sql);

  // Create a factory with no-op defaults (we provide publisher/subscriber directly)
  const factory = createResumableStreamContextFactory({
    publisher: () => publisher,
 ...
 */
export const createPgResumableStreamContext = createResumableStreamContext;
